...
public class AcceptorFactory extends BOAbstractFactory implements ITechFunction, ITechAcceptor {
   ...
   public Acceptor createAcceptor(ByteObject acc) {
      boolean accept = acc.hasFlag(ITechAcceptor.ACC_OFFSET_01_FLAG, ITechAcceptor.ACC_FLAG_1_ACCEPT);
      int op = acc.get1(ACC_OFFSET_02_OPERATOR1);
      int type = acc.get1(ACC_OFFSET_03_TYPE1);
      Acceptor acceptor = new Acceptor(boc, op, accept, type);

      int v = acc.get4(ACC_OFFSET_05_OPERAND4);
   ...
   public ByteObject getAcceptor(int acceptor, boolean accept, boolean countRejects) {
      ByteObject p = createAcceptorEmpty();
      p.setFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_1_ACCEPT, accept);
      p.setValue(ACC_OFFSET_05_OPERAND4, acceptor, 4);
      return p;
   }
   ...
   public ByteObject getAcceptor(int[] data, boolean isAcceptor) {
      ...
      p.setFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_8_OPERAND, true);
      p.setValue(ACC_OFFSET_03_TYPE1, ACC_TYPE_1_ARRAY, 1);
      p.addByteObject(boc.getLitteralIntFactory().getLitteralArray(data));
      return p;
   }

   private ByteObject createAcceptorEmpty() {
      return new ByteObject(boc, IBOTypesBOC.TYPE_022_ACCEPTOR, ACC_BASIC_SIZE);
   }
    ...
    * @param op
    * @param accRight
    * @param isAcceptor Sets the {@link ITechAcceptor#ACC_FLAG_1_ACCEPT}
    * @return
    */
   ...
   public ByteObject getAcceptorInt(int value, int condition, boolean acceptor) {
      ByteObject p = createAcceptorEmpty();
      p.setFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_1_ACCEPT, acceptor);
      p.setFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_8_OPERAND, true);
      p.setValue(ACC_OFFSET_05_OPERAND4, value, 4);
      p.setValue(ACC_OFFSET_02_OPERATOR1, condition, 1);
      p.setValue(ACC_OFFSET_03_TYPE1, ACC_TYPE_0_INT, 1);
   ...
   public ByteObject getAcceptorPointer(int operand, int op, ByteObject pointer, boolean acceptor) {
      ByteObject p = createAcceptorEmpty();
      p.setValue(ACC_OFFSET_03_TYPE1, ACC_TYPE_2_BYTEOBJECT, 1);
      p.setFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_8_OPERAND, true);
      p.setFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_1_ACCEPT, acceptor);

      p.setValue(ACC_OFFSET_05_OPERAND4, operand, 4);
      p.setValue(ACC_OFFSET_02_OPERATOR1, op, 1);
   ...
   public ByteObject getAcceptorString(String str, int operator, boolean acceptor) {
      int size = ACC_BASIC_SIZE;
      size += str.length() * 2;
      ByteObject p = new ByteObject(boc, IBOTypesBOC.TYPE_022_ACCEPTOR, size);
      p.setFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_1_ACCEPT, acceptor);
      p.setFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_8_OPERAND, true);
      p.setValue(ACC_OFFSET_02_OPERATOR1, operator, 1);
      p.setValue(ACC_OFFSET_03_TYPE1, ACC_TYPE_3_STRING, 1);
      p.setDynOverWriteChars(ACC_OFFSET_05_OPERAND4, str.toCharArray(), 0, str.length());
      return p;
   }
   ...
   public void toStringAcceptor(Dctx sb, ByteObject bo) {
      sb.append("#Acceptor");
      if (bo.hasFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_1_ACCEPT)) {
         sb.append(" accepts=");
      } else {
         sb.append(" rejects=");
      }
      if (bo.hasFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_3_ARRAY)) {
         int[] vals = bo.getValues(ACC_OFFSET_05_OPERAND4);
         boc.getUCtx().getIU().toStringIntArray(sb, vals, ",");
      } else {
         int val = bo.getValue(ACC_OFFSET_05_OPERAND4, 4);
         sb.append(boc.getUCtx().getColorU().toStringColor(val));
      }
      if (bo.hasFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_5_CHANNEL)) {
         sb.append(" 8bitsChannelMode");
      }
      if (bo.hasFlag(ACC_OFFSET_01_FLAG, ACC_FLAG_2_COUNT_REJECTS)) {
